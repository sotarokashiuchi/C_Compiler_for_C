...
Token_t* tokenize(void){
	...
	for(count=0, p=user_input; *p; count++){
		...
		if(!strncmp(p, "void", 4) && !is_alnum(p[4])){
			cur = new_token(TK_KEYWORD, cur, &p, 4);
			continue;
		}
		...
		// 2文字
		if(!strncmp(p, "<=", 2) || !strncmp(p, ">=", 2) || !strncmp(p, "==", 2) || !strncmp(p, "!=", 2) || !strncmp(p, "+=", 2) || !strncmp(p, "-=", 2) || !strncmp(p, "*=", 2) || !strncmp(p, "/=", 2) || !strncmp(p, "%=", 2) || !strncmp(p, "++", 2) || !strncmp(p, "--", 2) || !strncmp(p, "&&", 2) || !strncmp(p, "||", 2) || !strncmp(p, "->", 2)){
		...
		if(*p == '"'){
			...
			for(i=0; *tmpp != '"'; i++){
				if(*tmpp == '\\'){
					// エスケープ文字はアセンブラで翻訳されるため、そのままトークナイズしてよい
					tmpp++;
					i++;
				}
				tmpp++;
